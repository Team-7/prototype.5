...
public long insertBMI(int height, double weight, double score){//inserts a BMI record in database
	...
	if(myCursor.getCount() > 0){
		...
		if(getCenter(LoginActivity.getName())==1)
			myId = insertBMI(10001, height, weight, score);
		
		if(getCenter(LoginActivity.getName())==2)
			myId = insertBMI(20001, height, weight, score);
		
		if(getCenter(LoginActivity.getName())==3)
			myId = insertBMI(30001, height, weight, score);
	}
...
public int getCenter(String name){
	
	db = hopeHelper.getWritableDatabase();
	String [] columns = {HopeHelper.W_NAME, HopeHelper.CENTRE_ID};
	Cursor myCursor = db.query(HopeHelper.TABLE2, columns, HopeHelper.W_NAME+" = '"+name+"'", null, null, null, null);
	int theName = 0;
