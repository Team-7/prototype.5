...
public long insertBMI(String date, int height, double weight, double score){//inserts a BMI record in database
	...
	if(myCursor.getCount() > 0){
		...
		myId = insertBMI(10001, date, height, weight, score);
	}
...
public long insertChildBMI( int BMI_id, int child_id, int cnter_id){//links child to a bmi record (association)
...
public void clearAll(){
	db = hopeHelper.getWritableDatabase();
	db.execSQL("DELETE FROM "+HopeHelper.TABLE5);
	db.execSQL("DELETE FROM "+HopeHelper.TABLE4);
	...
	static class HopeHelper extends SQLiteOpenHelper{//Database class (Schema)
		...
		private static final String CREATE_TABLE4 = "CREATE TABLE "+ TABLE4+" ("+ BMI_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "+ DATE + " VARCHAR (10), "+
		...
		+ KEY_ID +" INTEGER (6) NOT NULL REFERENCES "+ TABLE1+"("+ KEY_ID+"), "+ CENTRE_ID +" INTEGER (3) NOT NULL REFERENCES "+ TABLE3+"("+ CENTRE_ID+"), PRIMARY KEY ("+KEY_ID+","+BMI_ID+"));";
